...
public class ApplicationInitializer implements WebApplicationInitializer {
    ...
    private void activateSpringProfile(
        ...
        try {
            configurableEnvironment.getPropertySources().addLast(new ResourcePropertySource(ENVIRONMENT_FILE));
            logger.info("Spring active profiles: {}", Arrays.toString(configurableEnvironment.getActiveProfiles()));
        } catch (IOException e) {
            // Properties file cannot be found.
            // We cannot ignore it!
            throw new IllegalStateException("Unable to initialize spring profiles: " + e.getMessage());
        }
